...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat, args):
        ...
        self.eval_location = args.eval_location
        self.each_ap = args.eval_each_ap
        self.out_dir = args.output_dir

        for hoi_pair in self.hico_triplet_labels:
            self.hoi_obj_list.append(hoi_pair[1])

        if not self.eval_location:
            ...
            def fetch_follower_likes(s_e):
                start, end, idx_iter = s_e
                cur_preds = preds[start:end]
                pred_inner = []
                for index, img_preds in enumerate(cur_preds):
                    img_preds = {k: v.to('cpu').numpy() for k, v in img_preds.items()}
                    bboxes = [{'bbox': list(bbox)} for bbox in img_preds['boxes']]
                    obj_scores = img_preds['obj_scores'] * img_preds['obj_scores']
                    hoi_scores = img_preds['hoi_scores'] + obj_scores[:, self.hoi_obj_list]
                    ...
                    filename = gts[index + start]['filename']
                    pred_inner.append({
                        'filename': filename,
                        'predictions': bboxes,
                        'hoi_prediction': hois
                    })
                collect[idx_iter] = pred_inner


            # fetch_follower_likes((1,100))
            pool_size = 20
            collect = {k: [] for k in range(pool_size)}
            mutex = threading.Lock()
            part_num = math.ceil(len(preds) / pool_size)
            # part_num = 6
            args_thread = zip(zip([(i * part_num, (i + 1) * part_num, i) for i in range(pool_size)]),
                       [None for i in range(pool_size)])
            pool = threadpool.ThreadPool(pool_size)
            my_requests = threadpool.makeRequests(fetch_follower_likes, args_thread)
            [pool.putRequest(req) for req in my_requests]
            pool.wait()
            pool.dismissWorkers(pool_size, do_join=True)

            for i in range(pool_size):
                self.preds += collect[i]
        else:
            self.preds = []
            for index, img_preds in enumerate(preds):
                img_preds = {k: v.to('cpu').numpy() for k, v in img_preds.items()}
                ...
                hois = []
                for idx in range(img_preds['sub_ids'].shape[0]):
                    hois.append(
                        {'subject_id': img_preds['sub_ids'][idx], 'object_id': img_preds['obj_ids'][idx], 'category_id': 0,
                         'score': 1.0})
        ...
        if not self.eval_location:
            ...
            def fetch_follower_likes(s_e):
                start, end, idx_iter = s_e
                cur_gts = gts[start:end]
                gts_inner = []
                for i, img_gts in enumerate(cur_gts):
                    filename = img_gts['filename']
                    img_gts = {k: v.to('cpu').numpy() for k, v in img_gts.items() if k != 'id' and k != 'filename'}
                    bbox_anns = [{'bbox': list(bbox), 'category_id': label} for bbox, label in
                                 zip(img_gts['boxes'], img_gts['labels'])]
                    ...
                    for hoi in gts_inner[-1]['hoi_annotation']:
                        triplet = hoi['category_id']
            ...
            collect = {k: [] for k in range(pool_size)}
            mutex = threading.Lock()
            part_num = math.ceil(len(gts) / pool_size)
            # part_num = 6
            args_thread = zip(zip([(i * part_num, (i + 1) * part_num, i) for i in range(pool_size)]),
                       [None for i in range(pool_size)])
            pool = threadpool.ThreadPool(pool_size)
    ...
    def evaluate(self):
        ...
        #         # bbox_pairs: gt bbox matched by pred bbox(miou > 0.5)
        #         # bbox_overlaps: score for gt bbox matched by pred bbox
        ...
        def fetch_follower_likes(s_e):
            start, end, idx_iter = s_e
            cur_preds = self.preds[start:end]
            cur_gts = self.gts[start:end]
            for img_preds, img_gts in zip(cur_preds, cur_gts):
                pred_bboxes = img_preds['predictions']
                if len(pred_bboxes) == 0: continue
        ...
        collect = {k: [] for k in range(pool_size)}
        mutex = threading.Lock()
        part_num = math.ceil(len(self.preds) / pool_size)
        # part_num = 6
        args_thread = zip(zip([(i * part_num, (i + 1) * part_num, i) for i in range(pool_size)]),
                          [None for i in range(pool_size)])
        pool = threadpool.ThreadPool(pool_size)
    ...
    def compute_map(self):
        ...
        if self.each_ap:
            ap_values = []
            for category_i in range(600):
                tuple_name = hico_text_label[self.hico_triplet_labels[category_i]]
                verb, obj = self.hico_triplet_labels[category_i]
                if category_i in list(ap.keys()):
                    score = ap[category_i]
                else:
                    score = 0
                ap_values.append((tuple_name, verb, obj, score))
            with open(os.path.join(self.out_dir, 'each_ap.json'), 'w') as f:
                json.dump(ap_values, f)
