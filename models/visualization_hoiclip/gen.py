...
class GEN(nn.Module):
    ...
    def forward(self, src, mask, query_embed_h, query_embed_o, pos_guided_embed, pos_embed, clip_model, clip_proj,
        ...
        ins_hs, _ = self.instance_decoder(ins_tgt, memory, memory_key_padding_mask=mask,
        ...
        inter_hs, weight = self.interaction_decoder(inter_tgt, memory, memory_key_padding_mask=mask,
        ...
        return h_hs, o_hs, inter_hs, clip_cls_feature, torch.zeros_like(clip_hoi_score).to(clip_hoi_score.device), \
               clip_visual @ clip_proj.to(dtype),  weight.unflatten(2, (h, w)), \
               torch.zeros(8, 64, 7, 7).to(o_hs.device)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), weight
